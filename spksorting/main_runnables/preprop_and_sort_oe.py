...
sys.path.insert(0, "../")
from preprocess_rhd.preprocess_oe import preprocess_one_session
from utils_here import clear_folder
MAP_CSV_PATH = "/media/hanlin/Liuyang_10T_backup/jiaaoZ/codes/geom_channel_maps/ChannelMap_Ben.csv"
ML_TMP_DIRECTORY = "/media/hanlin/Liuyang_10T_backup/jiaaoZ/ml_temp"

BAD_SESSIONS_RHD = []
def procfunc_prepdata(
    list_of_raw_folders : list, 
    list_of_mda_folders : list, 
    list_of_result_folders, 
    mpq : multiprocessing.Queue
    ):
    for i_work, (rawfoldername, mdafoldername, resultfoldername) in enumerate(zip(list_of_raw_folders, list_of_mda_folders, list_of_result_folders)):
        print(i_work)
        # info_dict={}
        ...
        # else:
        try:
            info_dict = preprocess_one_session(rawfoldername, mdafoldername)
        except Exception:
            traceback.print_exc()
            BAD_SESSIONS_RHD.append(rawfoldername)
            continue
...
def procfunc_sortdata(mpq : multiprocessing.Queue):
    while True:
        try:
            ...
            ml_temporary_directory = ML_TMP_DIRECTORY
            mapcsvpath = MAP_CSV_PATH
            if not os.path.exists(resultfoldername):
                os.makedirs(resultfoldername)
...
if __name__=="__main__":
    rawfolder_root = "/media/hanlin/Liuyang_10T_backup/jiaaoZ/data/BenMouse0-other"
    mdafolder_root = "/media/hanlin/Liuyang_10T_backup/jiaaoZ/data_converted/BenMouse0-other"
    resfolder_root = "/media/hanlin/Liuyang_10T_backup/jiaaoZ/msort_results/BenMouse0-other"
    print("Start")
