...
def stat_datapoints_by_period(dict_animals, duration_in_days, period_in_days):
    ...
    return means, std_errors, days_approx, nvals
...
if __name__ == "__main__":
    ...
    pd.DataFrame(
        data=tmp_datadict
    ).to_csv(os.path.join(PLOTFOLDER, "impedances_12weeks_all.csv"))
    # skip NaN values so that the plotted trace does not have gaps
    tmp_mask = (~np.isnan(means))
    means = means[tmp_mask]
    stdes = stdes[tmp_mask]
    days_approx = days_approx[tmp_mask]
    nvals = nvals[tmp_mask]
    # all animals
