...
def postprocess_one_session(session_folder):
    ...
    for i_clus in tmp_clus_ids:
        ...
        # print("Pimmary channels:", np.argmax(p2p_by_channel), prim_ch)
        p2p_near = p2p_by_channel>(PARAMS['P2P_SPREAD_RATIO']*p2p_prim)
        # print("i_clus=", i_clus, "#chs w/ large waveform=", np.sum(p2p_near))
        # print(p2p_by_channel)
        # print((geom[p2p_near,0]-prim_x)**2+(geom[p2p_near,1]-prim_y)**2)
    ...
    ####^^^^^^ FOR COROLLA SESSIONS THAT DID NOT THROW AWAY BAD CHANNELS ONLY
    prim_xs = geom[pri_ch_lut, 0]
    prim_ys = geom[pri_ch_lut, 1]
    assert len(prim_xs.shape)==1, prim_xs.shape[0]==n_clus
    assert len(prim_ys.shape)==1, prim_ys.shape[0]==n_clus
    np.logical_and(prim_ys>=200, prim_ys<=400)
    clus_in_noise_chs_mask = np.logical_and.reduce((prim_ys>=200, prim_ys<=400, prim_xs>12))
    cluster_accept_mask[clus_in_noise_chs_mask] = False
    ####^^^^^^
...
if __name__ == '__main__':
    error_sessions = []
    result_folder = "/media/hanlin/Liuyang_10T_backup/jiaaoZ/spinalcord/msort_results/corolla_chronic/"
    ...
    # postprocess_one_session("/media/hanlin/Liuyang_10T_backup/jiaaoZ/msort_results/corolla_chronic/Corolla_220201_180933")
