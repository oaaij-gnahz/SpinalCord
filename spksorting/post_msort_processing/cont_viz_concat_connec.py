...
def read_postproc_data(msort_path: str, postproc_path: str, read_connec: bool) -> dict:
    ...
    ret['prim_chs']
    Optional : ret['connectivity'] (n_connecs, 3)
    ...
    if read_connec:
        try:
            connecs = pd.read_csv(os.path.join(postproc_path, "mdas_screened", "connecs.csv"), header=None).values
            # print(connecs.shape)
            label_mapping = pd.read_csv(os.path.join(postproc_path, "mdas_screened", "map_clean2original_labels.csv"), header=None).values
            label_mapping = np.squeeze(label_mapping)
            # print(label_mapping.shape)
            connecs[:, :2] = label_mapping[connecs[:,:2]-1]
            # print(connecs.shape)
            ret['connectivity'] = connecs
        except pd.errors.EmptyDataError: # empty file implies no connectivity
            ret['connectivity'] = np.zeros((0, 3))
    return ret



def add_seg_to_axis(ax : matplotlib.axes.Axes, ix_offset : int, pp_dict : dict, geom : np.ndarray, draw_rejected : bool, draw_connec : bool):
    ...
    if draw_connec:
        for i_connec in range(pp_dict['connectivity'].shape[0]):
            src_label, snk_label, con_type = pp_dict['connectivity'][i_connec, :]
            con_color = ("blue" if con_type < 0 else "red")
            src_x, src_y = pp_dict['locations'][src_label-1, :2]
            snk_x, snk_y = pp_dict['locations'][snk_label-1, :2]
            plt.plot(
                [src_x+ix_offset*SEG_SPACING, snk_x+ix_offset*SEG_SPACING], 
                [src_y, snk_y], c=con_color
                )

    return spiking_mask, lambda_smap, scatter_accepted


def plot_main(postproc_folder : str, draw_tracking : bool, draw_rejected : bool, draw_connection : bool):
    ...
    for i_seg, foldername in enumerate(segment_folders[:-1]):
        print(i_seg)
        pp_dict = read_postproc_data(foldername, os.path.join(foldername, "customPostproc220509"), draw_connection)
        spiking_mask, lambda_smap, scatter_accepted = add_seg_to_axis(ax, i_seg, pp_dict, geom, draw_rejected, draw_connection)
    ...
    plt.savefig("tmp_connec.png")

plot_main(cont_root_path, draw_tracking=False, draw_rejected=False, draw_connection=True)
